  ...
  convert(data){
    ...
    for(let line of lines){
      ...
      if(this.pattern.test(line)){
        ...
        // should grab everything before and after otherwise forces <? on own line.
        open = true;
      }else if(/\?>/.test(line)){
        // same thing forces ?> on its own line.
        open = false;
      }else{
        // full lines
        if(open){
          doc = this.append(doc, line);
...
function handleTemplate(req, res, uri){
  ...
  eval(out+"function write(data){module.exports.t.write(data);}");
}
